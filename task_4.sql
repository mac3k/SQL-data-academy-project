...
FROM (
	SELECT a.year
		,ROUND(AVG(a.cat_avg_value_price), 2) AS avg_price
		,COALESCE(ROUND(AVG(b.cat_avg_value_price), 2), 'N/A') AS previous_y_price
		,ROUND((1-(AVG(b.cat_avg_value_price)/AVG(a.cat_avg_value_price)))*100, 2) AS yearly_pct_price_change
...
LEFT JOIN (
	SELECT a.`year`	
		,ROUND(AVG(a.ind_avg_payroll_value), 2) AS avg_payroll_value
		,a.ind_pay_unit 
		,COALESCE(ROUND(AVG(b.ind_avg_payroll_value), 2),'N/A') AS previous_y_payroll_value 
		,ROUND((1-(AVG(b.ind_avg_payroll_value)/AVG(a.ind_avg_payroll_value)))*100, 2) AS yearly_pct_payroll_change	
	FROM t_marcel_sufcak_project_SQL_primary_final a
	LEFT JOIN t_marcel_sufcak_project_sql_primary_final b
